...
import { getPokedexIndex, hydrateTypesForIds, getPokemon, artworkFor } from '@/lib/pokeapi';
import { useUIStore } from '@/stores/uiStore';
import { useTeamStore } from '@/stores/teamStore';
import { formatPokemonName, padId } from '@/lib/utils';
import type { PokemonListItem, TeamMember } from '@/types/pokemon';

export function PokemonPicker() {
  const open = useUIStore((s) => s.pickerOpen);
  const slot = useUIStore((s) => s.pickerSlot);
  ...
  const filtered = useMemo(() => {
    ...
    if (/^\d+$/.test(q)) {
      return all.filter((p) => String(p.id).includes(q)).slice(0, 80);
    }
    return all.filter((p) => p.name.includes(q)).slice(0, 80);
  }, [all, query]);
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                  {filtered.map((p) => (
                    <button
                      ...
                      className="group text-left p-3 rounded-xl glass card-hover disabled:opacity-50 disabled:cursor-wait"
                    >
